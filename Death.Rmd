...
#CDC <- data.frame( Year = c( 2020, 2021), COVID=c( 385668, 462556), 
#                   TD=c( 3390079, 3469560))
...
cat( "\nTotal Disrepancy")
TD <- sum( CDCdelta)
prettyNum( -TD, big.mark=",", trim=TRUE)
```
...
$$
D_a(w) = a(w - w_{min})^2 + D_{min}
$$

where $D_a$ are the average deaths in week $w$, $w_{min}$ is the week number of the minimum deaths, and $D_{min}$ is the average deaths in week $w_{min}$.

```{r DmDA}
Dm <- mean( DA$Mean)
wmin <- DA$week[which(DA$Mean == min( DA$Mean))]
DA <- DA %>% mutate( wa = week - wmin, AM = Mean - Dm)
(avgpnts <- ggplot( data=DA) + aes( x = wa, y = AM) + geom_point() +
      xlab( "Adjusted Week") + ylab( "Adjusted Mean"))
```

Week `r wmin` is hte minimum week.  There is a cyclical nature here.  It looks like it might be sharply peaked, but let us construct a data item that will allow us to see the means back-to-back.

```{r DA3}
DA3 <- rbind(DA, DA, DA) # replicate DA 3 times
DA3$nw <- seq_along(DA3$week)
ggplot( data=DA3) + aes( x=nw, y = AM) + geom_point() +
      xlab( "New Week") + ylab( "Adjusted Mean")
```

So a highly peaked approximation is called for, and using a polynomial over a year is well suited to this form.  The folowing will develop a range of polynomials.
...
for (degree in 2:dlim) {
   expr <- as.formula( paste("AM ~ poly( wa, degree = ", degree, ")"))
   fm <- lm( expr, data = DA ) # this ensures the degree shows up in the model
   #fm <- lm( AM ~ poly( wa, degree = degree), data = DA )
   var <- paste( "avgfit", degree, sep =  ".") # creates a variable "avgfit.n"
   assign( var, fm) # essentially avgfit.n <- fm
   varlst <- paste( varlst, ", ", var)
   avgfit <- rbind( avgfit, 
                    data.frame( wa = DA$wa, week = DA$week,
                                degree = rep_along( DA$wa, paste(degree)),
                                deg = rep_along( DA$wa, degree),
                                Pred=as.vector( fitted( get( var))))) 
                                 #as.vector strips names
}
avgpnts2 + geom_line( data = avgfit, 
                      aes( x = wa, y = Pred, group = degree, color = degree))
...
So model three (4th degree) looks like the most significant.  The way to interpret this is that model 3's RSS would only have a 0.04% chance of randomly being that much less than model 2's RSS, whereas model 4's RSS has a 12% chance of randomly being that much less than model 3's. Studyng the graph below reveals the wisdom in the statistics.


```{r}
avgpnts2 +  geom_line( data=avgfit[avgfit$deg %in% c(3,4,5),], 
                       aes(x=wa, y=Pred, group = degree, color=degree))

```

Now we need to apply this over the whole data set.  
...
DS$Pred <- avgfit$Pred[ avgfit$degree == 4][ DS$week] + 
   yt$Pred[ DS$Year - 2014] / 52 # Adds one week's worth of death for that year.
ltyp <- rep( "blank", 8)
(dpswl1 <- dpsw + 
      geom_line( data = DS, inherit.aes = FALSE, 
                 mapping = aes( x  = date, y = Pred)) +
      labs( title = "Weekly Deaths", subtitle = "Prediction ~ Poly(5) + Year",
            caption = "Weekly deaths from CDC database.  Prediction line estimated over 2015 - 2019."))
```
...
```{r}
dpsw + scale_x_datetime( 
   limits = as.POSIXct( c("12/01/21", "07/11/22"),
                                  format="%m/%d/%y")) 
```

Note that week nine of 2022 is below expectation.  One might think that the pandemic was over in week nine, but this would be mistaken.  The large number of preceding deaths means that once the pandemic is over, the death curve will match an earlier year (although, there are no longer *excess* deaths that can be attributed to COVID).  This looks like if happens on week 12 (the last week of March).

```{r totaldeath}
ggplot( data=DS[ DS$Year==2022,]) + 
   geom_point( aes(x=date, y=Total - Pred))
t <- DS[ DS$Year==2022,]
t2 <- t$Total - t$Pred
t2[ t2 >0]
td.2022 <- sum( t2[ t2 >0])
paste( "Total Deaths in 2022: ", td.2022, "\n")
paste( "Total COVID Death through week 11: ", ed[ "2020"]+ed["2021"] + td.2022)
```

## Alternate Method of Deaths by Week

That was a complicated process that can all be automated within `lm`.  We need to make an "adjusted week" columm and an adjusted total column which will have the mean subtracted out.  A fourth degree polynomlial will still be used.
...
DS$aw <- DS$week - wmin
fit2 <- lm( aTotal ~ poly( aw, degree = 4) + aYear, data = DS[ DS$Year < 2020,])
summary( fit2)
...
(dpsw3 <- dpsw2 + geom_line( data=DS, 
   ...
   labs( title = "Weekly Deaths", subtitle = "Prediction ~ Poly( week, 4) + Year",
            caption = "Weekly deaths from CDC database.  Prediction (black line) estimated over 2015 - 2019.") +  annotate( "rect", xmin = as.POSIXct( "03/23/22", format = "%m/%d/%y"),
             xmax = as.POSIXct("05/17/22", format = "%m/%d/%y"), ymin = 52500,
             ymax = 57500, alpha = 0.2) + theme( legend.position = "none"))
```

The coefficients are somewhat difficult to interpret, but note the `aYear` coefficient is `r coef(fit2)["aYear"]`, which is comparable to the previous value (~40,000) divided by 52 (`r m`).  Below is the difference between the methods.

```{r}
ggplot( data=DS, mapping=aes( x = date, y = (Pred - Pred2))) + geom_point()
```

## End of the COVID 19 Pandemic

When the deth curve starts to match the 4^th degree polynomial, the pandemic is truly over. Simply eyeballing the previous graphs, it looks like the death rate has settled back to the 2019 level.  The following will demonstrate this.
...
dpsw3 +  geom_line( data = DS2, mapping = aes( x = date, y = DS2$Pred2),
 ...
 caption = "Solid black line is 2022 prediction, dotted red line is 2017 prediction.") +
...
We can probably get a better estimate of excess deaths by looking at the weekly data.  The start will be the first week in 2020 in which the weekly deaths exceeded the prediction.

```{r}
DS <- DS[ order( DS$sw), ]
DS$cTotal <- cumsum( DS$Total - DS$Pred2)
plot( DS$sw, DS$cTotal, panel.first = grid())
fst <- which( (DS$Total > DS$Pred2) & DS$Year == 2020)[ 1]
...
DS$cTotal[ imax] - DS$cTotal[fst - 1] 
